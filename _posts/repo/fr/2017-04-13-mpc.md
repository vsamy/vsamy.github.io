...
# Un MPC, c'est quoi ?
MPC est l'acronyme de `Model Preview Control` ou en français `Modèle de Contrôle Prédictif`.
...
Et dans la plupart des cas, il est difficile d'en extraire une solution analytique dans le temps.
Et du coup, il faut discrétiser le système qui alors devient résoluble par le mpc.
...
convient. \\(\mathbf{s}\\) le vecteur d'état du système et \\(\mathbf{u}\\) le vecteur de contrôle.
...
Finalement, considérant une valeur désirée/une trajectoire désirée \\(\mathbf{X}\_T\\) on calcule sur l'horizon de temps choisi et considérant la fonction de coût suivante
...
## Le système
Le système est écrit dans le fichier `PreviewSystem.h`.
Il définit l'ensemble du système, alloue et construit les variables \\(\Phi\\), \\(\Psi\\) et \\(\mathbf{\xi}\\).
Il est nécessaire de créer un shared pointer pour ce système.
...
auto ps = std::make_shared<mpc::PreviewSystem>(A, B, d, x_0, x_traj, nrStep);
```

## Le mpc
Le mpc est quant à lui écrit dans le fichier `previewController.h`.
Il y a deux versions: `MPCTypeFull` et `MPCTypeLast`.
Dans la cas où on cherche à se rapprocher d'une trajectoire, il faut utiliser la première version.
Pour un point final désiré, le second doit être utilisé.
Créer le mpc est chose facile.

```c++
mpc::MPCTypeFull controller(ps);
// Construit le système et résout le problem
controller.solve();
```

Il est aussi possible de (ré)initialiser le mpc avec un autre système.

```c++
mpc::MPCTypeFull controller();
// (Ré)initialse le mpc avec un nouveau système
controller.initializeController(ps);
```

Pour récupérer les résultate, faites-en juste la demade :)
...
## Les contraintes
La librairie fournit plusieurs types de contraintes.
Elles peuvent toutes être crées avec matrice-pas/vecteur-pas ou matrice-total/vecteur-total.
Plus tard, on montrera qu'on peut aussi mélanger les deux.
Les contraintes doivent aussi être construites
...
Another way is to call the function `Needs to be reimplemented`

### The autoSpan function
The `autoSpan` function is present in all constraints.
This method needs to be called if you have created a constraint with step-matrix/vector and whole-matrix/vector.
...
This computes internally the time needed to uild the problem and to solve it.

### Prefer MPCTypeLast over MPCTypeFull for single target
The class `MPCTypeFull()` is really made for trajectory but it is possible to give a target. 
If you do so, the mpc will try to reach the target as fast as it can, so it may reach before the end of the horizon of time.
Whereas, `MPCTypeLast()` only tries to reach the target at the end of the horizon of time. 
Internally, `MPCTypeLast()` has less computation to do and thus, is faaster.
...
Don't forget the (re)initialize the mpc if you have changed the dimension of the problem.
...
In dynamic walk, a model preview control is used to find a trajectory for the Center of Mass (CoM) of the robot considering its Zero-momentum Point (ZMP). 
...
## Pymanoid
[Pymanoid]() is library developed in python by [Stéphane Caron]() which allows you to use the model preview controller. It is meant for humanoid robots and displays nice features in [openrave]().
