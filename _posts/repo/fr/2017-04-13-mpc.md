...
où \\(A\\), \\(B\\) et \\(\mathbf{d}\\) sont respectivement la matrice d'état, la matrice de contrôle et le biais. Pour comprendre comment discrétiser son système [rien de mieux qu'une page Wikipédia(mettre link fr)](https://en.wikipedia.org/wiki/Discretization)
...
Ainsi, on voit assez facilement (on passe la démonstration ici) qu'à la \\(N\\)-ième étape on a:
